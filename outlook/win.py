...
from outlook.base import OutlookItemType, OutlookFolderType, OutlookBodyFormat
from outlook.types import Email, EmailAttachment, EmailFolderType
...
def get_emails(
    ...
    def process_emails(_conn: OutlookConnection) -> list[Email]:
        if email_folder_type is not None:
            outlook_folder_type = OutlookFolderType.from_email_folder_type(
                email_folder_type
            )
        ...
        for message in folder_items:
            try:
                ...
                logger.warning(
                    "메일 정보를 가져오는 중 오류 발생 : %s", e, exc_info=True
                )
                continue
    ...
    def process_email(_conn: OutlookConnection) -> Email:
        ...
        if hasattr(message, "Attachments") and message.Attachments.Count > 0:
            for i in range(1, message.Attachments.Count + 1):
                ...
                os.remove(temp_path)
                attachments.append(
                    EmailAttachment(filename=filename, content_base64=content_base64)
                )
        return Email(
